...
int g_listabslen;                        // maximum length of an abstract text
int g_listabsobjnum;                     // maximum number of objects in an abstract text
int g_feedlistnum;                       // number of articles in a RSS feed
int g_feedlistabslen;                    // maximum length of an RSS abstract text
int g_feedlistabsobjnum;                 // maximum number of objects in an abstract text
...
static void setarthtml(TCMPOOL *mpool, TCMAP *cols, int64_t id, int bhl,
                       int abslen, int absobjnum, bool tiny);
...
static int realmain(int argc, char **argv){
  ...
  if(g_tmpl){
    ...
    if(tctmplload2(g_tmpl, tmplpath)){
      ...
      rp = tctmplconf(g_tmpl, "listabslen");
      g_listabslen = rp ? tcatoi(rp) : 256;
      rp = tctmplconf(g_tmpl, "listabsobjnum");
      g_listabsobjnum = rp ? tcatoi(rp) : -1;
      rp = tctmplconf(g_tmpl, "feedlistnum");
      g_feedlistnum = tclmax(rp ? tcatoi(rp) : 10, 1);
      rp = tctmplconf(g_tmpl, "feedlistabslen");
      g_feedlistabslen = rp ? tcatoi(rp) : 256;
      rp = tctmplconf(g_tmpl, "feedlistabsobjnum");
      g_feedlistabsobjnum = rp ? tcatoi(rp) : -1;
...
static void dosession(TCMPOOL *mpool){
  ...
  if(!strcmp(p_act, "files") && auth && post) omode = TDBOWRITER;
  ...
  if(!strcmp(p_act, "login")){
    ...
    if(p_id > 0){
          ...
          if(!strcmp(numbuf, p_hash)){
            if(*p_wiki != '\0'){
              ...
              if(!name || *name == '\0'){
                ...
                setarthtml(mpool, cols, p_id, 0, -1, -1, false);
      ...
      if(!name || *name == '\0'){
        ...
        setarthtml(mpool, cols, 0, 0, -1, -1, false);
        tcmapprintf(vars, "titletip", "[preview]");
        tcmapput2(vars, "view", "preview");
        tcmapputmap(vars, "art", cols);
        tcmapput2(vars, "wiki", p_wiki);
    ...
    if(rnum < 1){
      ...
      if(cols){
        setarthtml(mpool, cols, id, 0, -1, -1, false);
        if(checkfrozen(cols) && !admin){
          tcmapput2(cols, "frozen", "true");
        } else if(cancom){
          tcmapputkeep2(cols, "comnum", "0");
      ...
      for(int i = 0; i < rnum && i < max; i++){
        ...
        TCMAP *cols = tcmpoolpushmap(mpool, id > 0 ? dbgetart(tdb, id) : NULL);
        if(cols){
          setarthtml(mpool, cols, id, 1, -1, -1, true);
    ...
    int max = g_listnum;
    int abslen = g_listabslen;
    int absobjnum = g_listabsobjnum;
    if(!strcmp(p_format, "atom")) {
      max = g_feedlistnum;
      abslen = g_feedlistabslen;
      absobjnum = g_feedlistabsobjnum;
    }
    int skip = max * (p_page - 1);
    ...
    if(rnum < 1){
      ...
      for(int i = 0; i < rnum && i < max ; i++){
        ...
        TCMAP *cols = tcmpoolpushmap(mpool, id > 0 ? dbgetart(tdb, id) : NULL);
        if(cols){
          setarthtml(mpool, cols, id, 1, abslen, absobjnum, false);
          tclistpushmap(arts, cols);
        }
      }
      if(tclistnum(arts) > 0){
  ...
  if(g_sidebarnum > 0 && strcmp(p_format, "atom")){
    // side bar
    TCLIST *res = searcharts(mpool, tdb, NULL, NULL, "cdate", g_sidebarnum + 1, 0, true);
    int rnum = tclistnum(res);
    if(rnum > g_sidebarnum) {
      rnum = g_sidebarnum;
      tcmapput2(vars, "sideartsmore", "");
    }
    TCLIST *arts = tcmpoollistnew(mpool);
  ...
  const char *tmplstr = trimxmlchars(tcmpoolpushptr(mpool, tctmpldump(g_tmpl, vars)));
...
static void setarthtml(TCMPOOL *mpool, TCMAP *cols, int64_t id, int bhl,
  ...
  if(rp && *rp != '\0'){
    if(tiny){
      ...
      TCXSTR *xstr = tcmpoolxstrnew(mpool);
      int txtlen = wikitohtml(xstr, rp, idbuf, g_scriptname, bhl + 1, g_uploadpub,
                              abslen, absobjnum);
      if(tcxstrsize(xstr) > 0) {
        tcmapput(cols, "texthtml", 8, tcxstrptr(xstr), tcxstrsize(xstr));
        if(abslen > 0 && txtlen > abslen) tcmapput2(cols, "omitted", "true");
      }
    }
  }
  rp = tcmapget2(cols, "comments");
